...
static int SMTPHandleCmd_MAIL(const char *pszCommand, BSOCK_HANDLE hBSock, SMTPSession &SMTPS)
{
	...
	if (SMTPCheckReturnPath(pszCommand, ppszRetDomains, SMTPS, pszSMTPError) < 0) {
		...
		if (pszSMTPError != NULL) {
			...
			SMTPSendError(hBSock, SMTPS, "501 Internal server error while checking return path");
		}
		return ErrorPop();
	}
	StrFreeStrings(ppszRetDomains);
	...
	if (iDomainCount == 1) {
		...
		if (USmlIsCmdAliasAccount(pszRealDomain, szDestUser) == 0) {
			...
			if (SMTPCheckRelayCapability(SMTPS, szDestDomain) < 0) {
				...
				SysInetRevNToA(SMTPS.PeerInfo, szPeerAddr, sizeof(szPeerAddr));
				SysLogMessage(LOG_LEV_DEBUG, "%s for IP address %s(reversed)\n", pszSMTPError, szPeerAddr);
...
static int SMTPHandleCmd_RCPT(const char *pszCommand, BSOCK_HANDLE hBSock, SMTPSession &SMTPS)
{
	...
	if (SMTPCheckForwardPath(ppszFwdDomains, SMTPS, pszSMTPError) < 0) {
		...
		if (pszSMTPError != NULL) {
			...
			SMTPSendError(hBSock, SMTPS, "501 Internal server error while checking forward path");
		}
...
static int SMTPDoAuthPlain(BSOCK_HANDLE hBSock, SMTPSession &SMTPS, char const *pszAuthParam)
{
	...
	if (Base64Decode(pszAuthParam, (int)strlen(pszAuthParam), szClientAuth, &iDec64Length) != 0) {
		SMTPSendError(hBSock, SMTPS, "501 Syntax error in parameters or arguments");
...
static int SMTPDoAuthLogin(BSOCK_HANDLE hBSock, SMTPSession &SMTPS, char const *pszAuthParam)
{
	...
	if (pszAuthParam == NULL || IsEmptyString(pszAuthParam)) {
		...
		Base64Encode(LOGIN_AUTH_USERNAME, (int)strlen(LOGIN_AUTH_USERNAME),
	...
	Base64Encode(LOGIN_AUTH_PASSWORD, (int)strlen(LOGIN_AUTH_PASSWORD), szPassword, &iEnc64Length);
	...
	if (Base64Decode(pszAuthParam, (int)strlen(pszAuthParam), szDecodeBuffer,
			 &iDec64Length) != 0) {
		SMTPSendError(hBSock, SMTPS, "501 Syntax error in parameters or arguments");
	...
	iDec64Length = sizeof(szDecodeBuffer);
	if (Base64Decode(szPassword, (int)strlen(szPassword), szDecodeBuffer, &iDec64Length) != 0) {
		SMTPSendError(hBSock, SMTPS, "501 Syntax error in parameters or arguments");
...
static int SMTPDoAuthCramMD5(BSOCK_HANDLE hBSock, SMTPSession &SMTPS, char const *pszAuthParam)
{
	...
	Base64Encode(SMTPS.szTimeStamp, (int)strlen(SMTPS.szTimeStamp), szChallenge, &iEnc64Length);
	...
	if (Base64Decode(szChallenge, (int)strlen(szChallenge), szClientResp, &iDec64Length) != 0) {
		SMTPSendError(hBSock, SMTPS, "501 Syntax error in parameters or arguments");
