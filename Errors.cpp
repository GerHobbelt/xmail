...
int ErrSetErrorCode(int iError, char const *pszInfo, int iSize)
{
	...
	if (pszInfo != NULL) {
		...
		if (iError >= 0 && iError < ERROR_COUNT) {
			...
			pEV->pszInfo[iError] = (char *) StrMemDup(pszInfo, iSize, 1); /* [i_a] */
			if (ERR_TIMEOUT != pEV->iErrorNo)
			{
    		SysLogMessage(LOG_LEV_DEBUG, "%s%s%s\n", 
				ErrGetErrorString(pEV->iErrorNo), 
				(pEV->pszInfo[iError] ? ": " : ""),
				(pEV->pszInfo[iError] ? pEV->pszInfo[iError] : ""));
			}
		}
	}
	else if (ERR_TIMEOUT != pEV->iErrorNo)
	{
		SysLogMessage(LOG_LEV_DEBUG, "%s\n", ErrGetErrorString(pEV->iErrorNo));
	}
...
char *ErrGetErrorStringInfo(int iError)
{
	...
	int iInfoLength = (pEV->pszInfo[iErrIndex] != NULL) ? (int)strlen(pEV->pszInfo[iErrIndex]): 0;
