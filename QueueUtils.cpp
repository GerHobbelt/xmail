...
{
	int iVarLength;
	int iLenght = (int)strlen(pszVarName);
				    ...
				    char const *pszServer, char const *pszLogFile)
{
	...
	bool bNoBounceToReplyAddress = SvrTestConfigFlag("NoBounceToReplyAddress", false, hSvrConfig); /* [i_a] */

	QMSG_HANDLE hMessage;

	if (!bNoBounceToReplyAddress) { /* [i_a] */
		hMessage = QueCreateMessage(hSpoolQueue);
