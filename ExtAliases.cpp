...
int ExAlAddAlias(ExtAlias * pExtAlias)
{
	...
	if (pAliasFile == NULL) {
		...
		ErrSetErrorCode(ERR_EXTALIAS_FILE_NOT_FOUND, szAliasFilePath);
		return ERR_EXTALIAS_FILE_NOT_FOUND;
	}

	char szAliasLine[ALIAS_TABLE_LINE_MAX] = "";

	while (MscGetConfigLine(szAliasLine, sizeof(szAliasLine) - 1, pAliasFile) != NULL) {  /* [i_a] */
...
ExtAlias *ExAlGetAlias(char const *pszRmtDomain, char const *pszRmtName)
{
	...
	ExtAlias *pExtAlias = ExAlGetAliasFromStrings(ppszTabTokens, szAliasFilePath);
...
int ExAlRemoveAlias(ExtAlias * pExtAlias)
{
	...
	while (MscFGets(szAliasLine, sizeof(szAliasLine) - 1, pAliasFile) != NULL) {
		if (szAliasLine[0] == TAB_COMMENT_CHAR) {
			fprintf(pTmpFile, "%s\n", szAliasLine);
			continue;
		}
...
ExtAlias *ExAlGetFirstAlias(EXAL_HANDLE hLinksDB)
{
	...
	while (pExtAlias == NULL &&
	       MscGetConfigLine(szAliasLine, sizeof(szAliasLine) - 1, pGLSD->pDBFile) != NULL) {
		char **ppszStrings = StrGetTabLineStrings(szAliasLine);
		...
		// if (iFieldsCount >= ealMax)      [i_a]
		pExtAlias = ExAlGetAliasFromStrings(ppszStrings, szAliasLine); /* [i_a] */
...
ExtAlias *ExAlGetNextAlias(EXAL_HANDLE hLinksDB)
{
	...
	while (pExtAlias == NULL &&
	       MscGetConfigLine(szAliasLine, sizeof(szAliasLine) - 1, pGLSD->pDBFile) != NULL) {  /* [i_a] */
		char **ppszStrings = StrGetTabLineStrings(szAliasLine);
